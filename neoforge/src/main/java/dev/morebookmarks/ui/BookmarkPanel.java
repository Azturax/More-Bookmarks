...
public final class BookmarkPanel {
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY) {
		hoveredTooltip = null;
		Ui.fill(graphics, x, y, width, height, 0xD0121A24);
		Ui.border(graphics, x, y, width, height, 0xFF2E5A72);

		Font font = Minecraft.getInstance().font;
		graphics.drawString(font, Ui.tr("morebookmarks.panel.header"), x + PAD, y + PAD, 0xFFE8C36A, false);

		List<String> bookmarks = ModBookmarkManager.getBookmarks();
		int rowY = y + PAD + HEADER;
		if (bookmarks.isEmpty()) {
			graphics.drawString(font, Ui.tr("morebookmarks.panel.empty"), x + PAD, rowY + 2, 0xFF667788, false);
			return;
		}
		...
		for (int i = start; i < end; i++) {
			...
			if (hover) {
				Ui.fill(graphics, x + 1, rowY, width - 2, ROW, 0x553D8CFF);
				hoveredTooltip = bookmark;
			}
			drawEntry(graphics, font, bookmark, x + PAD, rowY + 2, width - PAD * 2);
			rowY += ROW;
		}
	}
	...
	private static void drawEntry(GuiGraphics graphics, Font font, String bookmark, int x, int y, int maxWidth) {
		String shown = font.plainSubstrByWidth(bookmark, maxWidth);
		if (!shown.equals(bookmark) && shown.length() > 1) {
			shown = shown.substring(0, shown.length() - 1) + "…";
		}
		if (shown.startsWith("@")) {
			graphics.drawString(font, "@", x, y, 0xFF5555FF, false);
			graphics.drawString(font, shown.substring(1), x + font.width("@"), y, 0xFFD0D8E8, false);
		} else {
			graphics.drawString(font, shown, x, y, 0xFFD0D8E8, false);
		}
	}
}
