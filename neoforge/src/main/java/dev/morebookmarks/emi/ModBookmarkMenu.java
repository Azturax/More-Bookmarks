...
/**
 * Popup anchored to {@link ModBookmarkButton}: add the current search,
 * list saved bookmarks with remove buttons, and optional clear-all.
 */
public final class ModBookmarkMenu {
	...
	public void layout(int buttonX, int buttonY, int screenWidth, int screenHeight) {
		...
		y = buttonY - 2 - height;
		if (y < 2) {
			y = buttonY + ModBookmarkButton.SIZE + 2;
		}
		clampScroll();
	}
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY) {
		hoveredTooltip = null;
		Ui.fill(graphics, x, y, WIDTH, height, 0xF010141C);
		Ui.border(graphics, x, y, WIDTH, height, 0xFF3D6B8A);

		Font font = Minecraft.getInstance().font;
		int rowY = y + PAD;

		boolean canAdd = ModBookmarkManager.canBookmark(ModBookmarkSearch.current());
		boolean addHover = Ui.contains(x + PAD, rowY, WIDTH - PAD * 2, ROW, mouseX, mouseY);
		drawRow(graphics, font, x + PAD, rowY, WIDTH - PAD * 2, addHover && canAdd,
		...
		rowY += ROW + 2;
		Ui.fill(graphics, x + PAD, rowY - 1, WIDTH - PAD * 2, 1, 0xFF2A3A4A);
		...
		for (int i = start; i < end; i++) {
			...
			if (rowHover) {
				Ui.fill(graphics, x + PAD, rowY, WIDTH - PAD * 2 - 12, ROW, 0x663D8CFF);
			}
			drawBookmarkLabel(graphics, font, bookmark, x + PAD + 2, rowY + 3, WIDTH - PAD * 2 - 18);
			int removeColor = removeHover ? 0xFFFF6666 : 0xFFCC8888;
			graphics.drawString(font, "x", x + WIDTH - PAD - 9, rowY + 3, removeColor, false);
			if (rowHover) {
		...
		if (showClearAll()) {
			Ui.fill(graphics, x + PAD, rowY, WIDTH - PAD * 2, 1, 0xFF2A3A4A);
			rowY += 2;
			boolean clearHover = Ui.contains(x + PAD, rowY, WIDTH - PAD * 2, ROW, mouseX, mouseY);
			drawRow(graphics, font, x + PAD, rowY, WIDTH - PAD * 2, clearHover, 0xFFFFAAAA,
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		...
		if (Ui.contains(x + PAD, rowY, WIDTH - PAD * 2, ROW, mouseX, mouseY)) {
			if (ModBookmarkManager.add(ModBookmarkSearch.current())) {
				Ui.click();
			}
			return true;
		}
		...
		for (int i = start; i < end; i++) {
			...
			if (Ui.contains(x + PAD, rowY, WIDTH - PAD * 2 - 12, ROW, mouseX, mouseY)) {
				ModBookmarkSearch.apply(bookmark);
				Ui.click();
				ModBookmarkOverlay.INSTANCE.closeMenu();
				return true;
			}
			rowY += ROW;
		}
	...
	private static String addDisabledTooltip() {
		if (!ModBookmarkManager.isValid(ModBookmarkSearch.current())) {
	...
	private static void drawRow(GuiGraphics graphics, Font font, int x, int y, int width, boolean hover, int color, Component text) {
		if (hover) {
			Ui.fill(graphics, x, y, width, ROW, 0x663D8CFF);
		}
		graphics.drawString(font, text, x + 2, y + 3, color, false);
	}

	private static void drawBookmarkLabel(GuiGraphics graphics, Font font, String bookmark, int x, int y, int maxWidth) {
		String shown = font.plainSubstrByWidth(bookmark, maxWidth);
		...
		if (shown.startsWith("@")) {
			graphics.drawString(font, "@", x, y, atColor, false);
			graphics.drawString(font, shown.substring(1), x + font.width("@"), y, restColor, false);
		} else {
			graphics.drawString(font, shown, x, y, restColor, false);
		}
	}
}
