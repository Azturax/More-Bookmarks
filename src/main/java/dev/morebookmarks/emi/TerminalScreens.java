 ...
 * container bounds for {@link HandledScreen}s EMI skips (empty slot lists)
...
public final class TerminalScreens {
	...
	private static Bounds fromHandled(HandledScreen<?> screen) {
		HandledScreenAccessor access = (HandledScreenAccessor) screen;
		int extra = 0;
		if (screen instanceof RecipeBookProvider provider && provider.getRecipeBookWidget().isOpen()) {
			extra = 177;
		}
	...
	private static Bounds fromReflection(Screen screen) {
		...
		if (x == null) {
			x = firstField(screen, "x", "guiLeft");
		}
		if (y == null) {
			y = firstField(screen, "y", "guiTop");
		}
		if (width == null) {
			width = firstField(screen, "backgroundWidth", "xSize");
		}
		if (height == null) {
			height = firstField(screen, "backgroundHeight", "ySize");
		}
